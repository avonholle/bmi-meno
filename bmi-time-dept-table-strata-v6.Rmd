---
title: "BMI HR by menopause status -- table with HR estimates by strata of early life BMI overweight categories."
...
load(file="bmi-time2-v6-strata.RData") # nat.1, ov.1, pre.1, surg.1 objects from sample-bmi-time-dept-analyses2-strata-v6.Rmd, stratified by early life BMI (overweight: yes/no)
gc()

sort( sapply(ls(),function(x){object.size(get(x))})) 

# make list of the data objects
list.dat = list(nat.1, nat.0,
                ov.1, ov.0,
                pre.1, pre.0, 
                surg.1, surg.0)
...
models2.dat = within(models2.dat, {
  ...
  analysis = ifelse(grepl(".yes", name, fixed=T), "Yes",
                          ifelse(grepl(".no", name, fixed=T), "No",
...
head(models.dat.update)

t2 = models.dat.update %>%
  select(coef.ci, p.vals, coef.ci0, coef.ci2, p.vals2, meno, i.n, py, analysis) %>%
  pivot_wider(names_from=analysis, 
              values_from = c("coef.ci", "p.vals", "coef.ci0", "coef.ci2", "p.vals2", "i.n", "py"))
t2
...
save(t2.v6.strata, file="results-v6update-strata.RData")
...
aic.dat3 = within(aic.dat3, {
  ...
  analysis = ifelse(grepl(".yes", name, fixed=T), "Yes",
                          ifelse(grepl(".no", name, fixed=T), "No^a",
...
t2.v6 = t2
t3.v6 = t3
save(t2.v6, t3.v6, c6, file="results-v6update-strata.RData")
