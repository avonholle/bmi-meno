---
title: "Pooled analyses for BMI HR by menopause status"
...
with(df.ped, table(meno.cause.t.f,ped_status))
...
# how many events in counting process data vs original?

load( file="dat-fortable1.RData") # df from sample-bmi-time-dept-datahandling.Rmd.  df is one line per person vs one line per person time for df.ped

# # unique ids
setDT(df.ped)[, .(number_of_distinct_ids = uniqueN(censor_id))] # total unique ids
setDT(df)[, .(number_of_distinct_ids = uniqueN(censor_id))] # total unique ids

# number of events? both have 9956
colnames(df.ped)
...
# test code to get cases and person-time per model
mdat = df.ped %>% 
  filter(meno.cause.t.f == "Surgical menopause")

dim(mdat)
class(mdat)

setDT(mdat)[, .(number_of_distinct_ids = uniqueN(censor_id))] # total unique ids
head(mdat)
sum(mdat$tend-mdat$tstart) # person-years
...
# create data for analysis below
# =================================
...
attr(pam.strat$terms, ".Environment") <- NULL
...
# remove large data files in environment connected to regression objects
# source: https://stackoverflow.com/questions/42230920/saverds-inflating-size-of-object
attr(pam2$terms, ".Environment") <- NULL
attr(pam3$terms, ".Environment") <- NULL
attr(pam4$terms, ".Environment") <- NULL

summ.pam.strat = summary(pam.strat)[c("p.coeff","se", "n")]
summ.pam2 = summary(pam2)[c("p.coeff","se", "n")]
summ.pam3 = summary(pam3)[c("p.coeff","se", "n")]
summ.pam4 = summary(pam4)[c("p.coeff","se", "n")]

attr(summ.pam.strat$formula,".Environment") = NULL
attr(summ.pam2$formula,".Environment") = NULL
attr(summ.pam3$formula,".Environment") = NULL
attr(summ.pam4$formula,".Environment") = NULL
...
attr(vfit$formula,".Environment") = NULL

# t <- seq(40, 55, by = 1)
# plot(x = t, y = coef(vfit)["bmi"] + coef(vfit)["tt(bmi)"] * t,
#   type = "l", ylab = "Beta(t) for bmi", las = 1,
#   col = Set1[1])
...
#df.ped <- as_ped(Surv(tstart, tend, event) ~ meno_Age_s + meno_cause_s + cohort.f + meno.cause.f + ethnic.f + menop.age.baseline + BMI + bmi + censor_id, 
#                      data = df)
...
#NOTE: not possible to reduce teh size of the pam3 object above, so reduce size of term.df object resulting from pam3 object
term.df = term.df %>%
  select(tstart, tend, fit, ci_lower, ci_upper, t, cox.fit, pam0.fit, pam.l.fit) # reduce object size
...
term.df.update = term.df.update %>%
  select(tstart, tend, fit, ci_lower, ci_upper, t, cox.fit, pam.l.fit) # reduce object size
...
term.df2.update = term.df2.update %>%
  select(tstart, tend, fit, ci_lower, ci_upper) # reduce object size

# confidence intervals for linear
term.df.linear <- mdat %>% ped_info() %>% 
  add_term(pam4, term = "bmi") %>%
  mutate_at(c("fit", "ci_lower", "ci_upper"), funs(. / .data$bmi)) %>%
  mutate( t = (tstart-46) + (tend - tstart) ) # age range from 45 to 55

term.df.linear = term.df.linear %>%
  select(tstart, tend, fit, ci_lower, ci_upper) # reduce object size
...
attr(gg_tv$formula,".Environment") = NULL
attr(gg_tv.f$formula,".Environment") = NULL
attr(gg_tv.update$formula,".Environment") = NULL
...
list.of.stuff= list(test.mods=test.mods, 
              ...
              term.df.linear = term.df.linear) # estimates for linear model (16)
...
surg.1 = get.mods(menotype="Surgical menopause", Model.fun=cleanModel2) 
...
pre.1 = get.mods(menotype="Premenopausal", Model.fun=cleanModel2)  

#note: environment in function is getting saved.
# solution here: https://community.rstudio.com/t/r-regression-model-size-gets-larger-when-being-saved-within-a-function/87609

```

```{r}
# what data objects are in the environment and may be saved into the RData object
ls()

```

```{r, eval=T}
#rm(df.ped) # get rid of this object in the environment or it gets saved via saved formulas
# source: https://stackoverflow.com/questions/34132172/saving-a-single-object-within-a-function-in-r-rdata-file-size-is-very-large


rm(list = setdiff(ls(), c("nat.1", "ov.1", "pre.1", "surg.1"))) # take out any objects from the global environment besides what I want to save
gc()

save(nat.1, ov.1, pre.1, surg.1, 
     file="bmi-time2.RData")
# 
# save(pre.1,
#      file="pre-dat.RData")
# save(nat.1,
...
```{r, eval=T}
# for some reason, R on slurm is saving the environment from previous run. Remove all objects from this run so not in memory next time I open R environment.
rm(list=ls()) 
gc()
