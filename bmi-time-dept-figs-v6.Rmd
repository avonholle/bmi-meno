...
# Files created in sample-bmi-time-dept-analyses2-v6.Rmd in dir above.
# each object in bmi-time2-v6.RData has the following:

load(file="bmi-time2-v6.RData") # nat.1, ov.1, pre.1, surg.1 objects from sample-bmi-time-dept-analyses2-v6.Rmd 
...
ggsave(filename = "Original-v6.png",
       width = 10,
       height=4)
...
p.surg. = surg.1[[5]] + theme_bw(base_size=15) + 
  ...
  scale_x_continuous(limits=c(44,55),
                     breaks = c(44, seq(45,54,2), 55)) +
  ylab(NULL) + 
  xlab("Age (years)") + ggtitle("Uterectomy") 
p.surg.
...
ggsave(filename = "Updated-v6.png",
       width = 12,
       height=5)
```
...
ggsave(filename = "Updated-v6-natural.png",
       width = 6,
       height=5)
```
