---
title: "BMI HR by menopause status -- table with HR estimates for v7 cohorts only."
...
load(file="bmi-time2-v7update.RData") # nat.1, ov.1, pre.1, surg.1 objects sample-bmi-time-dept-analyses-unadj-v7.Rmd,, primary analyses
...
dat2.dt = data.table(models2.dat) %>% 
  select(meno, analysis, coef.ci0, coef.ci) %>%
...
load(file="v7-cohorts.RData")
#cohorts.in.v7
c7 = data.frame(cohorts.in.v7)
#c7
...
l7 = levels(factor(c7$cohorts.in.v7))
l7
```
...
c7$cohorts.in.v7 = factor(c7$cohorts.in.v7,
                         levels = l7,
...
save(t2.v7, t3.v7, c7, file="results-v7update.RData")
...
kable(t3,
      col.names = c("Menopause status", 
                    rep(c("Null", 'Constant', 'Linear', 'Spline'), 2)),
  ...
  #landscape() %>%
    add_header_above(c(" " = 1, 
                     "Full sample" = 4,
                     "No menopausal hormone therapy at entry" = 4)) %>%
  column_spec(1, width = "10em") %>%
