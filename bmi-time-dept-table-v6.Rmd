---
title: "BMI HR by menopause status -- table with HR estimates for v6 cohorts only."
...
load(file="bmi-time2-v6update.RData") # nat.1, ov.1, pre.1, surg.1 objects from sample-bmi-time-dept-analyses-unadj-v6.Rmd, primary analyses
...
t2 = models.dat.update %>%
  select(coef.ci, p.vals, coef.ci2, p.vals2, meno, i.n, py, analysis) %>%
  pivot_wider(names_from=analysis, 
              values_from = c("coef.ci", "p.vals", "coef.ci2", "p.vals2", "i.n", "py"))
t2
...
kable(t2[c("meno", 
      ...
      longtable=F,
      row.names=F,
      caption="BMI breast cancer hazard ratios$^a$ for constant HR model") %>% #https://stackoverflow.com/questions/46085067/r-markdown-table-caption-width-with-kable-and-longtable
  #landscape() %>%
  column_spec(1, width = "10em") %>%
  add_header_above(c(" " = 3,
                     "Model 1: Constant BMI" = 2,
                     "Model 2: Linear BMI$^c$" = 2,
                     " "=2,
                     "Model 3: Constant BMI" = 2,
                     "Model 4: Linear BMI$^c$" = 2)) %>%
  add_header_above(c(" " = 1, 
                     "Unadjusted" = 6,
                     "Adjusted for BMI 18-24 years" = 6)) %>%  
...
Studies in analytic sample:

```{r, results='markup'}

# the cohorts in this analysis

load(file="v6-cohorts.RData")
#cohorts.in.v6
c6 = data.frame(cohorts.in.v6)
#c6
...
l6 = levels(factor(c6$cohorts.in.v6))
...
c6$cohorts.in.v6 = factor(c6$cohorts.in.v6,
                         levels = l6,
                         labels = c('Breakthrough Generations Study', # 11
                                    ...
                                    "Women's Lifestyle and Health Study (Sweden)
", #31
                                    "Black Women's Health Study")) #34

kable(levels(c6$cohorts.in.v6),
      col.names = "Studies")
...
t2.v6 = t2
t3.v6 = t3
save(t2.v6, t3.v6, c6, file="results-v6update.RData")
