---
title: "BMI HR by menopause status -- table with HR estimates."
date: '2022-08-05'
output: 
  html_document:
...
# NOTE: data files on the Wine server at 
# W:\projects\bmi-menopause\consortium-analysis\consortium-analysis-2021-update

# Files created in sample-bmi-time-dept-analyses2.Rmd in dir above.
...
```{r, eval=F, include=F}
# original code when include all analyses in one table
load(file="bmi-time2-v6.RData") # nat.1, ov.1, pre.1, surg.1 objects from sample-bmi-time-dept-analyses2-v6.Rmd, adj for early life BMI
gc()
nat.v2 = nat.1; ov.v2 = ov.1; pre.v2 = pre.1; surg.v2 = surg.1
rm(nat.1); rm(ov.1); rm(pre.1); rm(surg.1)
gc()

load(file="bmi-time2-v7.RData") # nat.1, pre.1 objects from sample-bmi-time-dept-analyses2-v7.Rmd, restricting to women not taking HRT at baseline
nat.v3 = nat.1; pre.v3 = pre.1; 
rm(nat.1); rm(pre.1)
gc()
...
# make list of the data objects
list.dat = list(nat.v1, nat.v2, nat.v3,
                ov.v1, ov.v2,
                pre.v1, pre.v2, pre.v3,
                surg.v1, surg.v2)


names.meno = c(rep("nat", 3),
               rep("ov", 2),
               rep("pre", 3),
               rep("surg", 2))
names.meno

names.analysis = c("orig",
                   "early",
                   "hrt",
                   "orig",
                   "early",
...
dat.fig.wide = dat.fig.combine %>%
  select(tstart, name, coef.ci) %>%
  pivot_wider(names_from=tstart, values_from=coef.ci) %>%
  select(name, `45`, `50`, `54`) %>%
  rename("est.45"=`45`, "est.50"=`50`, "est.54"=`54`)
...
#      longtable=F,
  ...
  column_spec(c(4,6,8,9,10), width = "7em") %>%
...
kable(t2[c("meno", 
           ...
           'coef.ci_early', 'p.vals_early', 'coef.ci2_early', 'p.vals2_early',
           'coef.ci_hrt', 'p.vals_hrt','coef.ci2_hrt', 'p.vals2_hrt')],
      booktabs=T, 
      col.names = c("Menopause status", rep(c("HR (95% CI)", "p-value^a", 
                                              "HR (95% CI)", "p-value^b"), 3)),
      escape=F,
      longtable=F,
      row.names=F,
      caption="BMI breast cancer HRs for constant HR model") %>% #https://stackoverflow.com/questions/46085067/r-markdown-table-caption-width-with-kable-and-longtable
  #landscape() %>%
  column_spec(1, width = "10em") %>%
  add_header_above(c(" " = 1, 
                     ...
                     "Model 5: Constant BMI" = 2,
                     "Model 6: Linear BMI" = 2)) %>%
  add_header_above(c(" " = 1, 
                     "Unadjusted" = 4,
                     "Adjusted for BMI 18-24 years" = 4,
                     "Restricted to no HRT" = 4)) %>%  collapse_rows(columns =1)  %>%
  kable_styling(font_size=16, latex_options="scale_down") %>%
  add_footnote("^a p-value for Chi-square test for difference from null model (no BMI term). ^b p-value for Chi-square test for difference from constant BMI model. First two models represents the full sample with no adjustment. Models 3-4 adjust for  BMI reported at ages 18-24 years. ModelS 5-6 restrict the sample to women who report no hormone therapy use at study entry. All analyses are stratified by cohort.")
...
aic.dat3 = within(aic.dat3, {
  ...
  analysis = ifelse(grepl(".orig", name, fixed=T), "Unadjusted",
                          ifelse(grepl(".early", name, fixed=T), "Adjust for early BMI^a",
...
aic.dat3$analysis = factor(aic.dat3$analysis, 
                       levels = c("Unadjusted",
                                  "Adjust for early BMI^a",
                                  "Restrict to people not reporting HRT at entry"))
...
kable(t3,
      col.names = c("Menopause status", rep(c("Null", 'Constant', 'Linear', 'Spline'), 3)),
