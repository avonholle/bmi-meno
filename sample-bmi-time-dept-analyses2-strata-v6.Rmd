---
title: "Pooled analyses for BMI HR by menopause status. stratify by BMI 18-24, overweight status"
...
load(file="bmidat-pamm-v6.RData") # df.ped subset from sample-bmi-time-dept-datahandling.Rmd has  bmi.1824.overweight variable. 
...
get.mods = function(menotype, overweight, Model.fun) {
...
mdat2 = df.ped %>% 
  ...
  filter(bmi.1824.overweight==overweight) # overweight=1 is yes and =0 is no strata 
...
nat.1 = get.mods(menotype="Natural", overweight=1, Model.fun=cleanModel2)
ov.1 = get.mods(menotype="Medically induced loss of ovarian function", overweight=1, Model.fun=cleanModel2)
pre.1 = get.mods(menotype="Premenopausal", overweight=1, Model.fun=cleanModel2)  # premenopause does not work unless you restrict to cohorts with > 30 events
surg.1 = get.mods(menotype="Surgical menopause", overweight=1, Model.fun=cleanModel2) # NOTE that this fails due to no events (or no cohorts with events>30)

# NOT overweight strata ====================================

nat.0 = get.mods(menotype="Natural", overweight=0, Model.fun=cleanModel2)
ov.0 = get.mods(menotype="Medically induced loss of ovarian function", overweight=0, Model.fun=cleanModel2)
pre.0 = get.mods(menotype="Premenopausal", overweight=0, Model.fun=cleanModel2)  # premenopause does not work unless you restrict to cohorts with > 30 events
surg.0 = get.mods(menotype="Surgical menopause", overweight=0, Model.fun=cleanModel2) # NOTE that this fails due to no events (or no cohorts with events>30)
```

```{r, eval=T}

save(nat.0, ov.0, pre.0, surg.0,
     nat.1, ov.1, pre.1, surg.1,
     file="bmi-time2-v6-strata.RData")
