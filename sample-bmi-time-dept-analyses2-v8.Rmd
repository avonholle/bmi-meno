---
title: "Pooled analyses for BMI HR by menopause status. Adjust for large set of confounders"
...
<!-- NOTE: these analyses follow an internal review of analyses in January 2023 requesting adjustment for potential confounders (Katie O'Brien).
In other bmi manuscript (http://oncology.jamanetwork.com/article.aspx?doi=10.1001/jamaoncol.2018.1771), confounders were:
...
get.mods = function(menotype, Model.fun) {
  ...
  # menotype = "Premenopausal" # menotype = "Surgical menopause" #menotype="Medically induced loss of ovarian function" # for debugging
...
pam.strat <- bam(ped_status ~ s(tend) + 
DOB_YR + MENA_AGE + AGE_FB + BIRTH_NO + FAM_BC + TIME_SINCE_LB + cohort.f + s(tend, by=cohort.f),
        data = mdat, 
        offset = offset, 
        family = poisson, 
        method = "fREML", 
...
# Cox PH
vfit <- coxph(
  formula = Surv(tstart, tend, ped_status) ~ 
DOB_YR + MENA_AGE + AGE_FB + BIRTH_NO + FAM_BC + TIME_SINCE_LB + bmi + tt(bmi),# + strata(cohort.f), # NOTE: get an error if you do strata with time-dept bmi
  data    = mdat,
  tt      = function(x, t, ...) x * t)
coef(vfit)
...
# clean up analysis objects: pam.strat, pam2, pam3, vfit, pam2.f, pam3.f, vfit.f
